...
function updateBars(house) {
    ...
    if (house === "top") {
        filteredChars = characters.filter(character => character.power_ranking > 0);
    } else {
        filteredChars = characters.filter(character => character.house === house);
    }
    filteredChars.sort((a, b) => b.probability_of_survival - a.probability_of_survival);
    renderBars(filteredChars);
}
